...
Shopperz is a template for [Thelia](http://thelia.net) ecommerce solution
...
* Copy and Paste the default template and name `shopperz`
* replace the assets directory by the assets directory of this repo.
* Don't forget to change the activated template
* edit the ```layout.tpl``` file and replace ```{default_translation_domain domain='fo.default'}``` by ```{default_translation_domain domain='fo.shopperz'}```.
